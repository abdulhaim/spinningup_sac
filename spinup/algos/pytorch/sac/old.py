...
def sac(actor_critic=core.MLPActorCritic, ac_kwargs=dict(), seed=0,
    ...
    test_env = BugCrippledEnv(cripple_prob=1.0)

    env.cripple_prob = 1.0
    test_env.cripple_prob = 1.0
    ...
    prob_iteration = 0
    cripple_probs = [1.0, 0.66, 0.33, 0.0]
    ...
    for t in range(total_steps):
        ...
        if (t + 1) % steps_per_epoch == 0:
            ...
            if epoch % 600 == 0 and prob_iteration == 0:
                prob_iteration += 1
                env.cripple_prob = cripple_probs[prob_iteration]
                test_env.cripple_prob = cripple_probs[prob_iteration]

            if epoch % 800 == 0 and prob_iteration == 1:
                prob_iteration += 1
                env.cripple_prob = cripple_probs[prob_iteration]
                test_env.cripple_prob = cripple_probs[prob_iteration]

            if epoch % 1000 == 0 and prob_iteration == 2:
                prob_iteration += 1
                env.cripple_prob = cripple_probs[prob_iteration]
                test_env.cripple_prob = cripple_probs[prob_iteration]
...
if __name__ == '__main__':
    ...
    parser.add_argument('--exp_name', type=str, default='sac_bug_cripple_v1')
